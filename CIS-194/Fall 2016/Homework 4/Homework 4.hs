...
elemList x (Entry a as) | x == a = True
                        | otherwise = elemList x as
...
nth l n            | listLength l < n = error "list too short!"
nth (Entry x xs) n | n == 1 = x
...
isGraphClosed initial adjacent isOk = go Empty (Entry initial Empty)
    where
      go _ Empty = True
      go seen (Entry c cs) | c `elemList` seen = go seen cs
                           | not (isOk c) = False
      go seen (Entry c cs) = go (Entry c seen) (adjacent c `appendList` cs)
...
handleEvent (KeyPress key) (State c d bx l)
    | gameWon (nthMaze l) bx
    , l < listLength mazes
    , key == " " = loadMaze (l+1)
handleEvent _ (State c d bx l)
    | gameWon (nthMaze l) bx     = State c d bx l
...
drawState (State c d boxes l)
  = showWin (State c d boxes l)
  & atCoord c (player d)
  & pictureOfBoxes boxes
  & pictureOfMaze (nthMaze l)
